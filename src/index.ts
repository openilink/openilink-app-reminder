 ...
 * 1. 加载配置 → 创建 Store
 * 2. 收集所有工具 → 创建 Router
 * 3. 启动 Scheduler（每 10 秒检查到期提醒）
 * 4. 启动 HTTP Server，注册路由
 * 5. 监听 SIGINT/SIGTERM 优雅关闭
 */
...
const scheduler = new Scheduler({ store, getHubClient });
scheduler.start();
...
server.listen(Number(config.port), () => {
  ...
  console.log(`[app] 路由: POST /hub/webhook | GET /oauth/setup | GET /oauth/redirect | POST /oauth/redirect | GET /manifest.json | GET /health`);
