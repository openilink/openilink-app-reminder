...
describe("handleWebhook", () => {
  ...
  const defaultOpts = {
    ...
    onCommand: vi.fn().mockResolvedValue("命令执行结果"),
  ...
  it("正确处理 url_verification 握手", async () => {
  ...
  it("正确签名的 command 事件返回结果", async () => {
    const body = JSON.stringify({
      ...
      event: {
        ...
        data: { command: "create_reminder", args: { content: "喝水", time: "5m" }, sender: { id: "user-001" } },
      },
    });
    const timestamp = String(Math.floor(Date.now() / 1000));
    const signature = sign(webhookSecret, timestamp, body);
    const req = mockRequest("POST", body, {
      "x-timestamp": timestamp,
    ...
    const onCommand = vi.fn().mockResolvedValue("提醒已创建");
    ...
    expect(res._statusCode).toBe(200);
    const parsed = JSON.parse(res._body);
    expect(parsed.reply).toBe("提醒已创建");
  });
});
